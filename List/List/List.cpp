...
*/
bool list::PushForward(const int val)
{
	if (head == nullptr)
	{
		try
		{
			head = new node(val);		// Попытка выделения памяти в списке с переданным параметром
	...
	else
	{
		try
		{
			node* curr = new node(val, head);	// Попытка выделения памяти под узел с переданным параметром
...
*/
int* list::operator[](const unsigned int index)
{
	...
	if (head)
	{
		...
		while (curr->next)
		{
			...
			if (!foundElem)			// По условию задачи, если элемент не будет найден
				curr->value = 0;	// То он заменяется нулем

			foundElem = false;
			curr = curr->next;
		}
		curr->value = 0;		// Последний элемент обязательно будет нулем
	}
	else
	{
